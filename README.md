...
画面の輝度は `src/config.h` の `displayBrightness` で設定します。範囲は 0〜255（0 は消灯）、初期値は 64 です。変更後は再ビルド・書き込みしてください。
...
設定はすべて Git 管理外の `src/config.h` にまとめています。リポジトリには [`src/config.example.h`](src/config.example.h) の初期値だけを含めます。

- `wifiSsid` / `wifiPassword` に 2.4 GHz Wi-Fi の接続情報を設定してください。SSID が空なら Wi-Fi・時刻同期・夜間減光を無効にして、通常輝度で動作します。
- 起動時に Wi-Fi に接続し、認証なしの NTP で時刻を取得します。成功後は24時間ごとに再同期します。間隔は `timeSyncIntervalMs` で設定します。
- 接続・同期は合わせて60秒で打ち切り、失敗時は1時間後に再試行します（`timeSyncTimeoutMs` / `timeSyncRetryMs`）。成功時も失敗時も SNTP と Wi-Fi を停止します。待機中も測定・表示・警告処理を続けます。
- 同期先は `ntpServer`（初期値 `pool.ntp.org`）、タイムゾーンは `timeZone`（初期値 `JST-9`、日本時間）です。
- 日本時間の22時以上〜翌7時未満は輝度32、それ以外は64です。`nightStartHour` / `nightEndHour` / `nightBrightness` / `displayBrightness` で調整できます。開始と終了を同じ時刻にすると夜間減光を無効にします。
- 初回の同期成功までは通常輝度にします。同期後は ESP32 のシステム時計で時刻を進め、再同期に失敗しても減光を続けます。外付け RTC（BM8563）への保存・復元は行いません。
- 接続情報はファームウェアに含まれます。Git 管理からの除外は、端末やビルド成果物内の認証情報を暗号化するものではありません。

実機では、起動後の `Time synchronized: ...; Wi-Fi off`、夜間境界での `Display brightness: ...`、Wi-Fi 不通時の60秒後のタイムアウトと測定継続を確認してください。24時間後の再同期と1時間後の再試行も確認対象です。時間を短縮して確認する場合は設定値を変更し、確認後に戻してください。
