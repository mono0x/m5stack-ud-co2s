...
測定値は初回の有効データを即座に表示し、以降は前回の採用から60秒以上経過して最初に受信した有効データに更新します。同じ色の間は CO2・温度・湿度・測定値の Serial 出力を最大1分に1回更新します。CO2 の色が変わる場合は、悪化・改善のどちらも測定値・色・警告判定を即座に更新し、その時点から次の60秒を数えます。平均化は行いません。USB 受信とデータ停止の監視は常時継続します。
...
- 同じ色が続く間や、色が改善したときは鳴らしません。青・緑では鳴りません。初回・データ復帰後に採用した値も黄色以上なら鳴らします。色の変化は1分を待たずに警告判定へ反映します。
- 切断時は `Connect UD-CO2S`、初回受信前は `Waiting for data`、受信行が 10 秒途絶えると `Data timeout` と表示します。古い測定値を隠し、警告音を停止します。
- 範囲外の数値（CO2: 400〜10,000 ppm、湿度: 0〜100%、温度: −40〜70℃の範囲外）は即座に `Out of range`、形式不正や `NaN` / `Inf` は `Invalid data` と表示し、測定値を隠して警告音を停止します。これらの行が届き続ける間は `Data timeout` にせず、`STA` も再送しません。正常値に戻った後は従来の更新間隔・色変化の条件で表示を再開します。
- 接続切れ・データ停止の表示と警告音の停止は1分を待たずに反映します。復帰後も同じ色で前回採用から60秒未満は `Waiting for update` を表示して値を隠し、更新間隔を維持します。
