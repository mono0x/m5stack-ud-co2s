...
- 初期値は **1,000 ppm 以上で 200 ms の警告音**。警告中は 10 秒ごとに鳴り、900 ppm 以下で解除します。境界付近で警告が頻繁に切り替わるのを防ぐため解除値を別にしています。
...
しきい値・音量・警告間隔は [`src/config.h`](src/config.h) で変更し、再ビルドして書き込んでください。音量は 0〜255 です。初期しきい値はアプリの設定値です。
...
2. 画面の測定値が更新され、設定した濃度以上で音が鳴ること。音の確認時だけ `config.h` のしきい値と解除値を現在値より低くしても確認できます。確認後は元に戻します。
